...
export default function Authentication() {
    ...
    const SignUpCard = () => {
        ...
        const addresDetailRef = useRef<HTMLInputElement | null>(null);
        ...
        // button Click event handler : 주소 버튼 클릭 이벤트 처리 //
        const onAddressButtonClickHandler = () => {};

        // button Click  event handler : 다음 버튼 클릭 //
        ...
        // button Click event handler : 회원 가입 버튼 클릭 //
        const onSignUpButtonClickHandler = () => {};

        // button Click event handler : 로그인 링크 클릭 //
        const onSignInLinkClickHandler = () => {
            setView("sign-in");
        };

        // key down event handler : 이메일 키 다운 //
        const onEmailKeyDownHanlder = (event: KeyboardEvent<HTMLInputElement>) => {
            if (event.key !== "Enter") return;
        ...
        // key down event handler : 비밀번호 키 다운 //
        const onPasswordKeyDownHanlder = (event: KeyboardEvent<HTMLInputElement>) => {
            if (event.key !== "Enter") return;
            if (!passwordRef.current) return;
            passwordRef.current.focus();
